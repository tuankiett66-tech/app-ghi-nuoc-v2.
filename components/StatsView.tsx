...
export const StatsView: React.FC<StatsViewProps> = ({ customers, activeTab, onBack, onClosePeriod, onBackupHistory, onExport, onExportReserveFund, onExportVat, onSelectCustomer }) => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      revenue: list.filter(c => !c.isSubMeter).reduce((sum, c) => sum + (c.amount || 0), 0),
      debt: list.filter(c => !c.isSubMeter).reduce((sum, c) => sum + (c.oldDebt || 0), 0),
      paid: list.filter(c => !c.isSubMeter).reduce((sum, c) => sum + (c.paid || 0), 0),
      balance: list.filter(c => !c.isSubMeter).reduce((sum, c) => sum + (c.balance || 0), 0),
