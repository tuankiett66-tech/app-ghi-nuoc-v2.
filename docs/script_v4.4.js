 ...
 * Bảo toàn Lịch sử sử dụng (updatedAt), Ngày thay đồng hồ (installDate) và Đồng hồ phụ (isSubMeter)
 * 
 * Định dạng các Cột trên Google Sheets (Từ Cột A đến Cột R):
 * - Cột A (1): Mã KH
 * - Cột B (2): Tên
 * - Cột C (3): Địa chỉ
 * - Cột D (4): SĐT
 ...
 * - Cột R (18): Đồng hồ phụ (isSubMeter) (Checkbox TRUE/FALSE)
 */
...
function getSheetData(sheet) {
  ...
  // Đảm bảo trang tính có tối thiểu 18 cột để tránh lỗi chỉ số vượt giới hạn (Bổ sung isSubMeter ở cột R)
  ...
  for (var i = 4; i < data.length; i++) {
    ...
    // Column R (index 17): isSubMeter (Đồng hồ phụ)
...
function updateOrInsertData(sheet, dataToUpdate) {
  ...
  // Đảm bảo trang tính có tối thiểu 18 cột trước khi xóa hoặc ghi dữ liệu (cột R là isSubMeter)
  var maxCols = sheet.getMaxColumns();
  if (maxCols < 18) {
    sheet.insertColumnsAfter(maxCols, 18 - maxCols);
  }
  ...
  if (lastRow >= 5) {
    // Xóa nội dung từ cột A đến R (18 cột) để đảm bảo sạch sẽ trước khi ghi mới
